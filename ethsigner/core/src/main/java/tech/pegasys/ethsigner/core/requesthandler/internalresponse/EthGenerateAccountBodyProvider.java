...
public class EthGenerateAccountBodyProvider implements BodyProvider {

  private static final Logger LOG = LogManager.getLogger();

  private final Supplier<Set<String>> addressesSupplier;

  public EthGenerateAccountBodyProvider(final Supplier<Set<String>> addressesSupplier) {
  ...
  public JsonRpcBody getBody(final JsonRpcRequest request) {
    ...
    if (isPopulated(params) && isNotEmptyArray(params)) {
      LOG.info("eth_generateAccount should have no parameters, but has {}", request.getParams());
